...
struct pdf_processor
{
	 ...
	 * processors. */
	void (*pop_resources)(fz_context *ctx, pdf_processor *proc);
...
pdf_processor *pdf_new_run_processor(fz_context *ctx, fz_device *dev, fz_matrix ctm, const char *usage, pdf_gstate *gstate, fz_default_colorspaces *default_cs, int has_transparency);
...
/*
	Retrieve a pointer to the buffered resources.

	The reference is borrowed, and so must be kept by the caller.
*/
pdf_obj *pdf_buffer_processor_resources(fz_context *ctx, pdf_processor *bufproc);
