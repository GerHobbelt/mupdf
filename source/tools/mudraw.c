...
static int output_file_per_page = 0;		// TODO TODO TODO
...
static int usage(void)
{
	...
	fz_info(ctx,
		...
		"        limited to number of bands per page), where number arg is one of:\n"
		...
		"  -M -  Control spot/overprint rendering mode\n"
#if FZ_ENABLE_SPOT_RENDERING
		"     0 = No spot rendering\n"
...
static void dodrawpage(fz_context *ctx, fz_page *page, fz_display_list *list, int pagenum, int start, int interptime, const char *fname, int bg, fz_separations *seps, fz_navigation *pagenav, pdf_document *pdfout_, fz_output *out_)
{
	fz_rect mediabox;
	fz_device *dev = NULL;
	...
	else if (output_format->format == OUT_SVG)
	{
		...
		fz_try(ctx)
		{
			...
			opts.text_format = ((stext_options.flags & FZ_STEXT_NO_TEXT_AS_PATH) ? FZ_SVG_TEXT_AS_TEXT : FZ_SVG_TEXT_AS_PATH);
			opts.reuse_images = !(stext_options.flags & FZ_STEXT_NO_REUSE_IMAGES);
...
static void drawpage(fz_context *ctx, fz_document *doc, int pagenum, fz_navigation *pagenav)
{
	...
	fz_try(ctx)
	{
		...
		else
		{
			if (verbosity >= 1 || showfeatures || showtime || showmd5)
				fz_info(ctx, "page %d file %s features %s", pagenum, filename, features);
			dodrawpage(ctx, page, list, pagenum, start, 0, filename, 0, seps, pagenav, pdfout, out);
		}
...
static void bgprint_worker(void *arg)
{
	...
	do
	{
		...
		if (pagenum >= 0)
		{
			...
			fz_try(bgprint.ctx)
			{
				dodrawpage(bgprint.ctx, bgprint.page, bgprint.list, pagenum, start, bgprint.interptime, bgprint.filename, 1, bgprint.seps, bgprint.pagenav, bgprint.pdfout, bgprint.out);
#if FZ_ENABLE_PDF
	...
	if (*lc == 0 || *lc == 'l')
	{
		...
		if (num_configs > 0)
		{
			...
			for (config = 0; config < num_configs; config++)
			{
				...
				if (name)
					fz_scatnprintf(msgbuf, sizeof(msgbuf), " Name=\"%s\"", name);
				creator = pdf_layer_config_creator(ctx, pdoc, config);
				if (creator)
					fz_scatnprintf(msgbuf, sizeof(msgbuf), " Creator=\"%s\"", creator);
...
int main(int argc, const char** argv)
{
	...
	fz_getopt_reset();
	while ((c = fz_getopt(argc, argv, "qQp:o:F:R:r:w:h:fB:c:e:gG:Is:A:DiW:H:S:T:t:d:U:LvVPl:y:Yz:Z:M:NO:am:x:Xhj:J:Kb:k:n:C")) != -1)
	{
		switch (c)
		{
	...
	if (max_num_workers > 0)
	{
		...
		if (band_height < 0)
		{
			fz_error(ctx, "band height must be > 0\n");
			return EXIT_FAILURE;
		}
	...
	{
		fz_error(ctx, "number of threads must be > 0\n");
		return EXIT_FAILURE;
	}
	else if (band_height != 0)
	{
		fz_error(ctx, "banding without multiple threads is pointless\n");
		return EXIT_FAILURE;
	}
