...
static void ffi_PDFPage_createAnnotation(js_State *J)
{
	fz_context *ctx = js_getcontext(J);
	pdf_page *page = js_touserdata(J, 0, "pdf_page");
	enum pdf_annot_type type = ffi_toenumx(J, 1, PDF_ANNOT_UNKNOWN, pdf_annot_type_from_string);
	pdf_annot *annot = NULL;
...
int murun_main(int argc, const char** argv)
{
	...
	if (fz_optind < argc)
	{
		...
		if (js_try(J))
		{
			fz_error(ctx, "cannot initialize script arguments\n");
			js_freestate(J);
			fz_drop_context(ctx);
			return EXIT_FAILURE;
		}
