...
static int
check_for_fake_bold(fz_context *ctx, fz_stext_block *block, fz_font *font, int c, fz_point p, float size)
{
	fz_stext_line *line;
	fz_stext_char *ch;
	...
	for (; block != NULL; block = block->next)
	{
		...
		{
			if (block->u.s.down != NULL && check_for_fake_bold(ctx, block->u.s.down->first_block, font, c, p, size))
				return 1;
		}
	...
	else
	{
		...
		else if (fabsf(base_offset) <= PARAGRAPH_DIST)
		{
			...
			if (wmode == 0 && cur_line && dev->new_obj)
				if ((p.x - dev->start.x) > (dev->pen.x - dev->start.x) * 0.5f)
					new_para = 1;
			new_line = 1;
		}
