...
add_char_to_line(fz_context *ctx, fz_stext_device *dev, fz_stext_page *page, fz_stext_line *line, fz_matrix trm, fz_font *font, float size, int c, int glyph, fz_point *p, fz_point *q, int bidi, int color, int synthetic, int flags, int dev_flags)
{
	...
	if (dev->opts.flags & FZ_STEXT_GLYPH_BBOX)
	{
		...
		if (glyph >= 0)
		{
			fz_rect bbox = fz_bound_glyph(ctx, font, glyph, trm);
...
fz_add_stext_char_imp(fz_context *ctx, fz_stext_device *dev, fz_font *font, int c, int glyph, fz_matrix trm, float adv, int wmode, int bidi, int force_new_line, int flags)
{
	...
	if (cur_line && glyph == -1)
	{
		...
		add_char_to_line(ctx, dev, page, cur_line, trm, font, size, c, (dev->opts.flags & FZ_STEXT_ACCURATE_BBOXES) ? glyph : NON_ACCURATE_GLYPH, &dev->pen, &dev->pen, bidi, dev->color, 0, flags, dev->opts.flags);
		dev->lastbidi = bidi;
		dev->lastchar = c;
	...
	else
	{
		...
		else if (fabsf(base_offset) <= PARAGRAPH_DIST)
		{
			...
			if (wmode == 0 && cur_line && dev->new_obj)
				if ((p.x - dev->start.x) > (dev->pen.x - dev->start.x) * 0.5f && !dev->maybe_bullet)
					new_para = 1;
			new_line = 1;
		}
	...
	if (new_line && (dev->opts.flags & FZ_STEXT_DEHYPHENATE) && fz_is_unicode_hyphen(dev->lastchar) && dev->lastline != NULL)
	{
	...
	{
		add_char_to_line(ctx, dev, page, cur_line, trm, font, size, ' ', (dev->opts.flags & FZ_STEXT_ACCURATE_BBOXES) ? NON_ACCURATE_GLYPH_ADDED_SPACE : NON_ACCURATE_GLYPH, &dev->pen, &p, bidi, dev->color, add_space, flags, dev->opts.flags);
	...
	add_char_to_line(ctx, dev, page, cur_line, trm, font, size, c, (dev->opts.flags & FZ_STEXT_ACCURATE_BBOXES) ? glyph : NON_ACCURATE_GLYPH, &p, &q, bidi, dev->color, 0, flags, dev->opts.flags);
...
static void
fz_add_stext_char(fz_context *ctx, fz_stext_device *dev, fz_font *font, int c, int glyph, fz_matrix trm, float adv, int wmode, int bidi, int force_new_line, int flags)
{
	/* ignore when one unicode character maps to multiple glyphs */
	if (c == -1)
		return;

	if (dev->opts.flags & FZ_STEXT_ACCURATE_ASCENDERS)
...
current_clip(fz_context *ctx, fz_stext_device *dev)
{
	...
	if (dev->opts.flags & FZ_STEXT_CLIP_RECT)
...
do_extract(fz_context *ctx, fz_stext_device *dev, fz_text_span *span, fz_matrix ctm, int start, int end, int flags)
{
	...
	for (i = start; i < end; i++)
	{
		if (dev->opts.flags & (FZ_STEXT_CLIP | FZ_STEXT_CLIP_RECT))
		{
		...
		{
			if (dev->opts.flags & FZ_STEXT_USE_CID_FOR_UNKNOWN_UNICODE)
			{
				unicode = span->items[i].cid;
				flags |= FZ_STEXT_UNICODE_IS_CID;
			}
			else if (dev->opts.flags & FZ_STEXT_USE_GID_FOR_UNKNOWN_UNICODE)
			{
...
static void
flush_actualtext(fz_context *ctx, fz_stext_device *dev, const char *actualtext, int i, float adv)
{
	if (*actualtext == 0)
		return;
...
do_extract_within_actualtext(fz_context *ctx, fz_stext_device *dev, fz_text_span *span, fz_matrix ctm, metatext_t *mt, int flags)
{
	...
	flush_actualtext(ctx, dev, actualtext, i, 0);
...
fz_stext_begin_metatext(fz_context *ctx, fz_device *dev, fz_metatext meta, const char *text)
{
	...
	if (mt != NULL && meta == FZ_METATEXT_ACTUALTEXT)
		flush_actualtext(ctx, tdev, mt->text, 0, 0);
...
fz_stext_end_metatext(fz_context *ctx, fz_device *dev)
{
	...
	if (tdev->last.valid)
	{
		flush_actualtext(ctx, tdev, tdev->metatext->text, 0, 0);
		pop_metatext(ctx, tdev);
		tdev->last.valid = 0;
		return;
	}
...
fz_stext_close_device(fz_context *ctx, fz_device *dev)
{
	...
	if ((tdev->opts.flags & FZ_STEXT_DEHYPHENATE) && fz_is_unicode_hyphen(tdev->lastchar) && tdev->lastline != NULL)
...
fz_apply_stext_options(fz_context *ctx, fz_stext_options *opts, fz_options *string)
{
	...
	if (fz_has_option(ctx, string, "use-gid-for-unknown-unicode", &val))
	{
		opts->flags_conf_mask |= FZ_STEXT_USE_GID_FOR_UNKNOWN_UNICODE;
		if (fz_option_eq(val, "yes"))
	...
	if (fz_has_option(ctx, string, "lazy-vectors", &val))
	{
		opts->flags_conf_mask |= FZ_STEXT_LAZY_VECTORS;
		if (fz_option_eq(val, "yes"))
	...
	if (fz_has_option(ctx, string, "paragraph-break", &val))
	{
	...
	if (fz_has_option(ctx, string, "collect-styles", &val))
	{
		opts->flags_conf_mask |= FZ_STEXT_COLLECT_STYLES;
		if (fz_option_eq(val, "yes"))
		SETCLEARBOOL(opts->flags, b, FZ_STEXT_COLLECT_STYLES);
	}
	if (fz_has_option(ctx, string, "accurate-ascenders", &val))
	{
		opts->flags_conf_mask |= FZ_STEXT_ACCURATE_ASCENDERS;
		if (fz_option_eq(val, "yes"))
		SETCLEARBOOL(opts->flags, b, FZ_STEXT_ACCURATE_ASCENDERS);
	}
	if (fz_has_option(ctx, string, "accurate-side-bearings", &val))
	{
		opts->flags_conf_mask |= FZ_STEXT_ACCURATE_SIDE_BEARINGS;
		if (fz_option_eq(val, "yes"))
	...
	if (fz_has_option(ctx, string, "clip-rect", &val))
	{
		opts->flags_conf_mask |= FZ_STEXT_CLIP_RECT;
		if (val_is_rect(val, &opts->clip))
...
add_vector(fz_context *ctx, fz_stext_page *page, fz_stext_device *tdev, fz_rect bbox, uint32_t flags, uint32_t argb, int id, float exp)
{
	...
	if (tdev->opts.flags & (FZ_STEXT_CLIP_RECT | FZ_STEXT_CLIP))
	{
...
maybe_rect(fz_context *ctx, split_path_data *sp)
{
	...
	if (sp->dev->opts.flags & (FZ_STEXT_CLIP_RECT | FZ_STEXT_CLIP))
...
fz_new_stext_device_for_page(fz_context *ctx, fz_stext_page *page, const fz_stext_options *opts, int chapter_num, int page_num, fz_rect mediabox)
{
	...
	if ((dev->super.flags & FZ_STEXT_PRESERVE_IMAGES) == 0 && (dev->opts.flags & FZ_STEXT_IGNORE_ACTUALTEXT) != 0)
