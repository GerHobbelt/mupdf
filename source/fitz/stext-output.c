...
static void
print_blocks_as_html(fz_context* ctx, fz_output* out, fz_stext_block* block, int pagenum, fz_matrix ctm, const fz_stext_options* options, fz_link* first_link);
...
fz_print_stext_page_as_html_with_links(fz_context *ctx, fz_output *out, fz_stext_page *page, int pagenum, fz_matrix ctm, const fz_stext_options *options, fz_link *first_link, fz_navigation *first_navigation)
{
	...
	fz_write_printf(ctx, out, "<div id=\"page%d\" style=\"position:relative;%.1fpt;height:%.1fpt;background-color:white;\" data-mediabox=\"%R\">\n",
		pagenum,
		w, h,
		&mediabox);
	...
	print_blocks_as_html(ctx, out, page->first_block, pagenum, ctm, options, first_link);
...
static void
fz_print_stext_struct_as_html(fz_context *ctx, fz_output *out, fz_stext_block *block, int pagenum, fz_matrix ctm, const fz_stext_options* options, fz_link* first_link)
{
	const char *tag;
	...
	print_blocks_as_html(ctx, out, block->u.s.down->first_block, pagenum, ctm, options, first_link);
...
print_blocks_as_html(fz_context *ctx, fz_output *out, fz_stext_block *block, int pagenum, fz_matrix ctm, const fz_stext_options* options, fz_link* first_link)
{
	...
	for (; block; block = block->next)
	{
		if (block->type == FZ_STEXT_BLOCK_IMAGE && (!options || (options->flags & FZ_STEXT_PRESERVE_IMAGES)))
			...
			fz_print_stext_struct_as_html(ctx, out, block, pagenum, ctm, options, first_link);
...
static int
run_to_xhtml(fz_context* ctx, fz_output* out, fz_stext_block* block, int pagenum, int object_index, fz_matrix ctm, const fz_stext_options* options);
...
static int
fz_print_stext_table_as_xhtml(fz_context* ctx, fz_output* out, fz_stext_block* block, int pagenum, int object_index, fz_matrix ctm, const fz_stext_options* options)
{
	...
	fz_try(ctx)
	{
		...
		for (tr = grid->next; tr != NULL; tr = tr->next)
		{
			...
			for (td = tr->u.s.down->first_block; td != NULL; td = td->next)
			{
				...
				fz_write_string(ctx, out, ">");
				object_index = run_to_xhtml(ctx, out, td->u.s.down->first_block, pagenum, object_index, ctm, options);
				fz_write_printf(ctx, out, "</td>\n");
				for ( ; y0 < y1; y0++)
					for (x = x0; x < x1; x++)
					{
...
static int
fz_print_struct_as_xhtml(fz_context* ctx, fz_output* out, fz_stext_block* block, int pagenum, int object_index, fz_matrix ctm, const fz_stext_options* options)
{
	const char *tag;
	...
	{
		return fz_print_stext_table_as_xhtml(ctx, out, block->u.s.down->first_block, pagenum, object_index, ctm, options);
	}

	tag = html_tag_for_struct(block->u.s.down);

	fz_write_printf(ctx, out, "<%s idx=\"%d\"", tag, block->u.s.index);
	fz_write_printf(ctx, out, " raw=\"%s\" std=\"%s\"",
		block->u.s.down->raw, fz_structure_to_string(block->u.s.down->standard));
	fz_write_printf(ctx, out, ">\n");

	object_index = run_to_xhtml(ctx, out, block->u.s.down->first_block, pagenum, object_index, ctm, options);
...
run_to_xhtml(fz_context* ctx, fz_output* out, fz_stext_block* block, int pagenum, int object_index, fz_matrix ctm, const fz_stext_options* options)
{
	while (block)
	{
		...
		case FZ_STEXT_BLOCK_TEXT:
			...
			break;
		case FZ_STEXT_BLOCK_STRUCT:
			object_index = fz_print_struct_as_xhtml(ctx, out, block, pagenum, object_index, ctm, options);
			break;
		}
		object_index++;
		block = block->next;
...
fz_print_stext_page_as_xhtml(fz_context *ctx, fz_output *out, fz_stext_page *page, int pagenum, fz_matrix ctm, const fz_stext_options *options)
{
	...
	run_to_xhtml(ctx, out, page->first_block, pagenum, 1, ctm, options);
...
xml_write_char(fz_context *ctx, fz_output *out, int c)
{
	...
	default:
		if (c >= 32 && c < 127)
			...
			fz_write_printf(ctx, out, "&#x%x;", c);  // XML NCR: Numeric Character Reference
		break;
	}
}
	...
	while (block)
	{
		...
		case FZ_STEXT_BLOCK_TEXT:
			...
			for (line = block->u.t.first_line; line; line = line->next)
			{
				...
				fz_write_printf(ctx, out, "<line bbox=\"%R\" wmode=\"%d\" dir=\"%P\" flags=\"%d\"\n",
						&line->bbox,
						line->wmode,
						&line->dir,
						line->flags);
