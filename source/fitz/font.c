...
fz_new_font(fz_context *ctx, const char *name, int use_glyph_bbox, int glyph_count)
{
	...
	if (name)
	{
		...
		fz_strncpy_s(ctx, font->family, name, sizeof font->family);
	}
	else
	{
		fz_strncpy_s(ctx, font->name, "(null)", sizeof font->name);
		fz_strncpy_s(ctx, font->family, "(null)", sizeof font->family);
	}
...
fz_new_font_from_buffer(fz_context *ctx, const char *name, fz_buffer *buffer, int index, int use_glyph_bbox)
{
	...
	if (face->family_name)
		fz_strncpy_s(ctx, font->family, face->family_name, sizeof font->family);
...
fz_prepare_t3_glyph(fz_context *ctx, fz_font *font, int gid)
{
	...
	fz_try(ctx)
	{
		font->t3run(ctx, font->t3doc, font->t3resources, font->t3procs[gid], dev, fz_identity, NULL, NULL, font->t3transparency, NULL, NULL);
		fz_close_device(ctx, dev);
...
fz_render_t3_glyph_direct(fz_context *ctx, fz_device *dev, fz_font *font, int gid, fz_matrix trm, void *gstate, fz_default_colorspaces *def_cs, void *fill_gstate, void *stroke_gstate)
{
	...
	ctm = fz_concat(font->t3matrix, trm);
	font->t3run(ctx, font->t3doc, font->t3resources, font->t3procs[gid], dev, ctm, gstate, def_cs, font->t3transparency, fill_gstate, stroke_gstate);
}
