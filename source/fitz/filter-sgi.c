...
fz_open_sgilog16(fz_context *ctx, fz_stream *chain, int w)
{
	...
	fz_try(ctx)
	{
		...
		state->c = 0;
		state->w = w;
		state->temp = Memento_label(fz_malloc(ctx, w * sizeof(uint16_t), __FILE__, __LINE__), "sgilog16_temp");
...
fz_open_sgilog24(fz_context *ctx, fz_stream *chain, int w)
{
	...
	fz_try(ctx)
	{
		state->err = 0;
		state->w = w;
		state->temp = Memento_label(fz_malloc(ctx, w * 3, __FILE__, __LINE__), "sgilog24_temp");
		state->chain = fz_keep_stream(ctx, chain);
	}
	fz_catch(ctx)
	{
...
fz_open_sgilog32(fz_context *ctx, fz_stream *chain, int w)
{
	...
	fz_try(ctx)
	{
		...
		state->c = 0;
		state->w = w;
		state->temp = Memento_label(fz_malloc(ctx, w * sizeof(uint32_t), __FILE__, __LINE__), "sgilog32_temp");
		state->chain = fz_keep_stream(ctx, chain);
	}
	fz_catch(ctx)
	{
