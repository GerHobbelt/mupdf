...
fz_new_buffer(fz_context *ctx, size_t size)
{
	...
	fz_try(ctx)
	{
		b->data = Memento_label(fz_malloc(ctx, size, __FILE__, __LINE__), "fz_buffer_data");
	}
	fz_catch(ctx)
	{
		fz_free(ctx, b);
...
fz_resize_buffer(fz_context *ctx, fz_buffer *buf, size_t size)
{
	...
	buf->data = fz_realloc(ctx, buf->data, size, __FILE__, __LINE__);
	buf->cap = size;
	if (buf->len > buf->cap)
		buf->len = buf->cap;
}
...
fz_append_buffer(fz_context *ctx, fz_buffer *buf, fz_buffer *extra)
{
	...
	{
		buf->data = fz_realloc(ctx, buf->data, buf->len + extra->len, __FILE__, __LINE__);
