	...
	*out_res = NULL;
	...
	fz_try(ctx)
	{
		*out_buf = fz_new_buffer(ctx, 1024);
		...
		pdf_process_contents(ctx, top, doc, in_res, in_stm, NULL);
		pdf_close_processor(ctx, top);

		*out_res = pdf_keep_obj(ctx, pdf_buffer_processor_resources(ctx, proc_buffer));
...
pdf_filter_type3(fz_context *ctx, pdf_document *doc, pdf_obj *obj, pdf_obj *page_res, pdf_filter_options *options, pdf_cycle_list *cycle_up)
{
	...
	fz_try(ctx)
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			pdf_process_contents(ctx, top, doc, in_res, val, NULL);

			pdf_close_processor(ctx, proc_buffer);

			pdf_update_stream(ctx, doc, val, fz_new_buffer_from_buffer(ctx, buffer), 0);
		}

		pdf_dict_put(ctx, obj, PDF_NAME(Resources), pdf_buffer_processor_resources(ctx, proc_buffer));
	}
	fz_always(ctx)
	{
		for (i = 0; i < num_filters; i++)
...
void pdf_filter_page_contents(fz_context *ctx, pdf_document *doc, pdf_page *page, pdf_filter_options *options)
{
	...
	{
		if (options->end_page)
			options->end_page(ctx, buffer, options->opaque);
