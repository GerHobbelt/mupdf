...
pdf_out_push_resources(fz_context *ctx, pdf_processor *proc, pdf_obj *res)
{
	...
	if (p->res != NULL)
		fz_throw(ctx, FZ_ERROR_GENERIC, "Can't push resources with output processor");
	p->res = pdf_keep_obj(ctx, res);
}
...
pdf_obj *
pdf_buffer_processor_resources(fz_context *ctx, pdf_processor *proc)
{
	pdf_output_processor *p = (pdf_output_processor *)proc;

	if (p->super.push_resources != pdf_out_push_resources)
		fz_throw(ctx, FZ_ERROR_GENERIC, "Not a buffer processor!");
