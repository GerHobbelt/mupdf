...
static void
pdf_default_op_Tj(fz_context *ctx, pdf_processor *proc, const char *str, size_t len)
{
	PASSON(op_Tj)(ctx, proc->chain, str, len);
}

static void
pdf_default_op_squote(fz_context *ctx, pdf_processor *proc, const char *str, size_t len)
{
	PASSON(op_squote)(ctx, proc->chain, str, len);
}

static void
pdf_default_op_dquote(fz_context *ctx, pdf_processor *proc, float aw, float ac, const char *str, size_t len)
{
...
static void
pdf_init_csi(fz_context *ctx, pdf_csi *csi, pdf_document *doc, pdf_lexbuf *buf)
{
...
pdf_try_load_font(fz_context *ctx, pdf_document *doc, pdf_resource_stack *rdb, pdf_obj *font)
{
...
pdf_process_extgstate(fz_context *ctx, pdf_processor *proc, pdf_csi *csi, pdf_obj *dict)
{
	...
	if (pdf_is_array(ctx, obj) && proc->op_Tf)
	{
		...
		if (pdf_is_dict(ctx, font_ref))
			font = pdf_try_load_font(ctx, csi->doc, proc->rstack, font_ref);
		else
	...
	case B('T','f'):
		if (proc->op_Tf)
		{
			...
			if (pdf_is_dict(ctx, fontobj))
				font = pdf_try_load_font(ctx, csi->doc, proc->rstack, fontobj);
			else
				font = pdf_load_hail_mary_font(ctx, csi->doc);
			fz_try(ctx)
...
void
pdf_process_raw_contents(fz_context *ctx, pdf_processor *proc, pdf_document *doc, pdf_obj *stmobj)
{
	pdf_csi csi;
	pdf_lexbuf buf;
	fz_stream *stm = NULL;
...
void
pdf_process_contents(fz_context *ctx, pdf_processor *proc, pdf_document *doc, pdf_obj *in_res, pdf_obj *stmobj, pdf_obj **out_res)
{
	if (in_res)
		pdf_processor_push_resources(ctx, proc, in_res);
	fz_try(ctx)
		pdf_process_raw_contents(ctx, proc, doc, stmobj);
	fz_always(ctx)
	{
		if (in_res)
		{
