...
fz_debug_html_flow(fz_context *ctx, fz_output* out, fz_html_flow *flow, int level)
{
	...
	while (flow)
	{
		...
		fz_write_printf(ctx, out, " script=%d", flow->script);
...
fz_debug_html_box(fz_context *ctx, fz_output *out, fz_html_box *box, int level)
{
	while (box)
	{
		...
		if (box->type == BOX_BLOCK || box->type == BOX_TABLE || box->type == BOX_TABLE_CELL) {
			...
			{
				indent(level+1);
				fz_write_printf(ctx, out, ">background-color=#%02x%02x%02x%02x\n",
			...
			{
				indent(level+1);
				fz_write_printf(ctx, out, ">border=(%g %g %g %g) #%02x%02x%02x%02x\n",
