...
** Windows builds default to a hard-coded path for `devenv.com` in
`+scripts/wrap/__main__.py+`. This can be overridded with `-b --devenv <devenv.com-location> ...`.

** Windows builds require `py`, the Python launcher. This is an option when installing Python, called "Install launcher for all users (recommended)"; see https://docs.python.org/3/using/windows.html.
