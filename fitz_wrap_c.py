...
{
    if (!color or (!PySequence_Check(color) && !PyFloat_Check(color))) {
        *n = 1;
        return;
    }
    ...
    for (i = 0; i < len; i++) {
        ...
        if (!INRANGE(mcol[i], 0, 1) or rc == 1) mcol[i] = 1;
    }
...
#endif
    fprintf(stderr, "JM_BinFromBuffer\n");
    if (!buffer) {
...
fz_buffer *JM_compress_buffer(fz_context *ctx, fz_buffer *inbuffer)
{
    ...
    fz_try(ctx) {
        ...
        if (data == NULL or compressed_length == 0)
            return NULL;
