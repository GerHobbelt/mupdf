...
def make_outparam_helper_csharp(
    ...
    if make_csharp_wrapper:
        ...
        # (generate by us) which is at top-level scope, so it's perhaps a
        # little odd that swig generates things as it does. But it has been
        # told to put everything inside a 'mupdf' namespace and C# only support
        # functions inside classes. Maybe swig could omit the 'namespace mupdf'
        # for these functions, but then it would have to have a single mupdf
        # class for all its generated functions, which might be complicate the
        # code generation.
        #
        if not return_void:
            write(f'var ret = ')
