...
namespace tesseract {
	...
	float DotProductAVX_512(const float* u, const float* v, int n) {
		...
		__m512 tmpf = _mm512_add_ps(t0, _mm512_shuffle_f32x4(t0, t0, _MM_SHUFFLE(0, 0, 3, 2)));
		__m128 r = _mm512_castps512_ps128(_mm512_add_ps(tmpf, _mm512_shuffle_f32x4(tmpf, tmpf, _MM_SHUFFLE(0, 0, 0, 1))));
		r = _mm_hadd_ps(r, r);
		float result = _mm_cvtss_f32(_mm_hadd_ps(r, r));

		alignas(32) float tmp[16];
		_mm512_store_ps(tmp, t0);
		result = tmp[0] + tmp[1] + tmp[2] + tmp[3] + tmp[4] + tmp[5] + tmp[6] + tmp[7] + tmp[8] + tmp[9] + tmp[10] + tmp[11] + tmp[12] + tmp[13] + tmp[14] + tmp[15];
		for (unsigned k = 0; k < rem; k++) {
			result += *u++ * *v++;
		}
		return result;
